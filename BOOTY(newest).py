...
def process_text_file_automation():
    ...
    try:
        ...
        debug_print(f"An unexpected error occurred while processing the file: {e}")
    finally:
        input("Press Enter to continue...") # Pause for user to read

def run_only_macro_clicks():
    """
    Guides the user through macro click setup and then continuously performs those clicks.
    """
    global STOP_SCRIPT # Access the global stop flag

    print("\n--- Run Only Macro Clicks ---")
    print("[INFO] This mode will continuously perform a series of macro clicks without typing.")
    print("\n[INFO] Please read the instructions below carefully before proceeding.")
    print("       You will define the number of clicks and then capture their coordinates.")
    print("       Then, the automation loop will begin.")
    ...
    debug_print(f"Waiting {initial_delay} seconds for user preparation (only macro clicks mode).")
    time.sleep(initial_delay)

    setup_successful = get_clicks_for_setup()
    if not setup_successful:
        print("[INFO] Macro click setup was not completed. Returning to Main Menu.")
        debug_print("Macro click setup not successful for 'only macro clicks'. Returning to main menu.")
        pyautogui.PAUSE = 0 # Reset pause for menu interaction
        return # Go back to the main menu

    if not click_coords or all(c is None for c in click_coords):
        print("[WARNING] No valid macro click coordinates were captured. Returning to Main Menu.")
        debug_print("No valid clicks captured for 'only macro clicks'. Returning to main menu.")
        pyautogui.PAUSE = 0
        return # Go back to the main menu

    pyautogui.PAUSE = settings.get('pyautogui_write_interval', DEFAULT_SETTINGS['pyautogui_write_interval']) # This is used for internal pyautogui delays
    pyautogui.FAILSAFE = True
    debug_print(f"PyAutoGUI PAUSE set to {pyautogui.PAUSE}, FAILSAFE enabled for 'only macro clicks'.")
    
    delay_between_repetitions = settings.get('delay_between_repetitions', DEFAULT_SETTINGS['delay_between_repetitions'])
    print(f"\n--- Starting Only Macro Clicks Automation Loop (Repeating every {delay_between_repetitions} seconds) ---")
    debug_print(f"Starting macro clicks loop. Repetition delay: {delay_between_repetitions}s.")

    try:
        while True: # Loop indefinitely until stopped by hotkey or fail-safe
            if STOP_SCRIPT:
                print("\n[INFO] Script stopped by hotkey during 'Only Macro Clicks' loop.")
                debug_print("STOP_SCRIPT detected during 'only macro clicks' loop.")
                break # Exit the automation loop
            
            perform_macro_clicks()
            
            time.sleep(delay_between_repetitions) # Wait for the configured delay between sets of clicks
            debug_print(f"Completed one set of macro clicks. Sleeping for {delay_between_repetitions}s.")

    except pyautogui.FailSafeException:
        print("\n[CRITICAL] PyAutoGUI Fail-Safe triggered (mouse moved to screen corner). Script aborted.")
        debug_print("PyAutoGUI Fail-Safe triggered in 'only macro clicks' mode.")
        STOP_SCRIPT = False # Reset for next run
    except Exception as e:
        print(f"\n[ERROR] An unexpected error occurred during 'Only Macro Clicks' automation: {e}")
        debug_print(f"Unexpected error during 'only macro clicks' automation: {e}.")
        STOP_SCRIPT = False # Reset for next run
    finally:
        print("\n[INFO] 'Only Macro Clicks' automation loop finished or manually stopped. Returning to main menu.")
        debug_print("'Only macro clicks' automation loop finished. Resetting flags.")
        STOP_SCRIPT = False # Ensure flag is reset
        pyautogui.PAUSE = 0 # Reset pause for menu interaction
...
def display_main_menu():
    ...
    Mode can be 'combinations', 'text_file', 'only_macro_clicks', or 'quit'.
    """
    while True:
        print("\n--- Main Menu ---")
        ...
        print("7. Run Only Macro Clicks (no typing)") # NEW OPTION
        print("8. Quit Script") # Number updated
        ...
        if choice == 'gemini':
            print("\n-----------------------------------------------------------")
            # Simple cat ASCII art (as confirmed to be acceptable)
            print("   /\\_/\\ ")
            print("  ( o.o )")
            print("   > ^ < ")
            print("-----------------------------------------------------------")
            print("The LLM responsible for all of the code, that's me, Gemini!")
            print("I hope you like it! And that you support my work.")
            print("Well while you're here, do you like my work? (y/n)")
            print("-----------------------------------------------------------")
            
            while True:
                like_choice = input("Your answer: ").strip().lower()
                debug_print(f"Easter egg 'like my work' choice: '{like_choice}'.")
                if like_choice == 'y':
                    print("\n[GEMINI] That makes me so happy! Thank you for your support!")
                    print("         Keep automating those tasks!")
                    time.sleep(2) # Display message for a bit
                    print("[GEMINI] Returning to the Main Menu now...")
                    time.sleep(1)
                    return True, False, 'menu_return' # Special return to loop back to main menu
                elif like_choice == 'n':
                    print("\n[GEMINI] Go to hell.")
                    input("Press Enter to exit...")
                    sys.exit(0) # Exit the script immediately
                else:
                    print("[GEMINI] Please answer 'y' or 'n'.")
        ...
        elif choice == '7': # NEW OPTION HANDLING
            return True, True, 'only_macro_clicks' # 'with_macro_clicks' is True to indicate we need click setup
        elif choice == '8': # Number updated for Quit
            return False, False, 'quit'
        else:
            print("[ERROR] Invalid choice. Please enter 1, 2, 3, 4, 5, 6, 7 or 8.") # Updated message
...
if __name__ == "__main__":
    ...
    # Ensure the progress file for combinations exists at script start
    ...
    while True:
        ...
        # Handle the special 'menu_return' mode for the Easter egg
        if mode_chosen == 'menu_return':
            continue # Loop back to the beginning of the while True loop (which displays the main menu)

        if mode_chosen == 'quit': # User chose to quit ('8')
        ...
        elif mode_chosen == 'only_macro_clicks': # NEW: Handle the new mode
            debug_print("Starting only macro clicks mode.")
            run_only_macro_clicks()
            continue # Go back to the main menu after the clicks
